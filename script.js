...
function renderCards(sites) {
    ...
    sites.forEach(site => {
        ...
        let isUnderConstruction = !site.url || site.url.trim() === '';

        let titleZh = site.title.zh;
        let titleEn = site.title.en;
        let btnTextZh = "進入網站";
        let btnTextEn = "Enter Site";

        if (isUnderConstruction) {
            btnTextZh = "施工中...";
            btnTextEn = "Under Construction...";
            a.href = "javascript:void(0)";
            a.className = 'card disabled-card';
        } else {
            a.href = site.url;
            a.className = 'card';
        }
        ...
        a.innerHTML = `
            ...
            <div class="card-subtitle">Prof. Alinggo Lee</div>
            <span class="btn-primary">
                <span class="bilingual">
                    <span class="lang-zh">${btnTextZh}</span>
...
function renderFooter(footerItems) {
    ...
    topRow.className = 'footer-row';
    const bottomRow = document.createElement('div');
    bottomRow.className = 'footer-row';
    ...
    footerItems.forEach(item => {
        ...
        `;
        if (item.labelZh === '聯繫' || item.labelEn === 'Contact') {
